...
export default async function AvailabilityPage() {
  ...
  // Transform to BookedBlock formar
  const bookedBlocks: BookedBlock[] = await activeBookings.map((booking) => ({
    id: booking._id,
  ...
  // Filter out busy times that overlap with our bookings (Avoid duplication)
          ...
          <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-sm text-muted-foreground">
            ...
            {bookedBlocks.length > 0 && (
              <>
                ...
                <span className="flex items-center gap-1">
                  <span className="inline-block h-3 w-3 bg-red-500 rounded" />
                  Declined
                </span>
                <span className="flex items-center gap-1">
