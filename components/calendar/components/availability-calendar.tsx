...
export function AvailabilityCalendar({
  ...
  const handleSave = () => {
    startSaveTransition(async () => {
      try {
        ...
        // Update the local state with real IDs from Sanity
  ...
  const onBlockSelect = (block: CalendarEvent) => {
    ...
    // Don't allow removing busy block
  ...
  return (
    <div className="relative h-[calc(100vh-180px)] min-h-[400px] sm:min-h-[600px]">
      ...
      {/* Sae/Discard Bar - shows when there are unsaved changes */}
      {hasChanges && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-white flex items-center gap-3 rounded-full px-4 py-2 shadow-xl border">
