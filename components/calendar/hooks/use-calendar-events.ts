...
import type { SlotInfo, TimeBlock, TimeBlockInteraction } from "../types";
...
const blocksAreEqual = (a: TimeBlock[], b: TimeBlock[]) => {
...
export function useCalendarEvents(initialBlocks: TimeBlock[] = []) {
  ...
  const copyDayToWeek = (
    ...
    for (let i = 0; i < 7; i++) {
      // Do not include the selected day.
      // Events in the selected day will be included to the ones it does not exist in.
  ...
  const clearWeek = (referenceDate: Date) => {
    ...
    setEvents((prev) => {
      return prev.filter((b) => b.start < weekStart || b.start >= weekEnd);
    });
  };
  ...
  // Called adter successfull save - update both events and savedBlocks
